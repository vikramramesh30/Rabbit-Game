...
int Rabbit::health() const
{
    return m_health;
}

int Rabbit::poisionCount() const
{
    return poision_count;
}
bool Rabbit::isPoisioned() const
{
    return m_poisoned;
}
...
void Arena::moveRabbits()
{
      ...
      {
        m_rabbits[i]->move();
        if(m_player->col() == m_rabbits[i]->col() && m_player->row() == m_rabbits[i]->row())
        {
          m_player->setDead();
        }
...
bool attemptMove(const Arena& a, int dir, int& r, int& c)
{
    if(!(a.player()->move(dir) == "Player couldn't move; player stands."))
    {
        switch (dir)
        {
            case NORTH:
...
bool checkIfSafe(const Arena& a, int r, int c)
{
    
    return a.numberOfRabbitsAt(r-1, c) ==0 && a.numberOfRabbitsAt(r, c + 1) ==0 && a.numberOfRabbitsAt(r + 1, c) == 0 && a.numberOfRabbitsAt(r, c-1) == 0;
    
}

int checkMostRabbits(const Arena& a, int r, int c)
{
    return a.numberOfRabbitsAt(r-1, c) + a.numberOfRabbitsAt(r, c + 1) + a.numberOfRabbitsAt(r + 1, c) + a.numberOfRabbitsAt(r, c-1);
}
...
bool recommendMove(const Arena& a, int r, int c, int& bestDir)
{
    ...
    int minRabbits = 1;
    int minDirection = NORTH;
    
    if(a.numberOfRabbitsAt(r-1, c) >=1 || a.numberOfRabbitsAt(r, c+1) >=1
       || a.numberOfRabbitsAt(r+1, c) >=1 || a.numberOfRabbitsAt(r, c-1) >= 1)
    {
        if(a.numberOfRabbitsAt(r-1, c) == 0)
        {
            if(checkIfSafe(a, r-1, c))
            {
                bestDir = NORTH;
                return true;
            }
            
            if(checkMostRabbits(a, r-1, c) <= minRabbits)
            {
                minRabbits = checkMostRabbits(a, r-1, c);
                minDirection = NORTH;
            }
        }
        if(a.numberOfRabbitsAt(r, c+1) == 0)
        {
            if(checkIfSafe(a, r, c+1))
            {
                bestDir = EAST;
                return true;
            }
